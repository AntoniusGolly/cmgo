...
CM.generatePolygon <- function(object){
  ...
  for(set in names(data)){
    ...
    if(is.null(data[[set]]$polygon)
      ...
      if(par$bank.reduce){
        ...
        banks_red = list(
          x = list(left = c(banks$x$left[1]), right = c(banks$x$right[1])),
          y = list(left = c(banks$y$left[1]), right = c(banks$y$right[1]))
        )

        # linearly interpolate bank points
        for(bank in c("left", "right")){

          for(i in c(2:length(banks$x[[bank]]))){

            # calculate distance between consecutive bank points
            cb_dist = ( ( banks$x[[bank]][i] - banks_red$x[[bank]][length(banks_red$x[[bank]])] )^2 + ( banks$y[[bank]][i] - banks_red$y[[bank]][length(banks_red$y[[bank]])] )^2 ) ^(1/2)
            ...
            if(cb_dist < par$bank.reduce.min.dist){
              ...
              # add the final point
              banks_red$x[[bank]] = append(banks_red$x[[bank]], banks$x[[bank]][i])
              banks_red$y[[bank]] = append(banks_red$y[[bank]], banks$y[[bank]][i])
        ...
        # the dense polygon will be generated by adding all left bank points, adding all right bank points in reversed order
        data[[set]]$polygon = ppp(
          c(banks_red$x$left, rev(banks_red$x$right)),
          c(banks_red$y$left, rev(banks_red$y$right)),
          window=owin(
            range(banks_red$x, na.rm=TRUE),
            range(banks_red$y, na.rm=TRUE)
          )
        )
  ...
  notice("polygon(s) generated successfully!", TRUE)
